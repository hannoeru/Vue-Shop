<template>
  <q-page class="q-pa-lg">
    ...
    <div class="row justify-start" v-if="$route.path !== '/checkout'">
      <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3" v-for="item in products" :key="item.id">
        <ProductCard :item="item" />
      </div>
    </div>
    <q-separator class="full-width q-ma-xl" v-if="$route.path !== '/checkout'" />
    <div class="row justify-center">
      <div class="col-4">
        <Cart />
        <Coupon />
        <div class="row justify-center q-my-xl" v-if="$route.path !== '/checkout'">
          <q-btn color="primary" label="下一步" @click="scrollToElement($refs.form)" />
        </div>
      </div>
    </div>
